...
SELECT COUNT(*) AS 資料表目前所有用戶數量 FROM "USER";
...
(
    (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io'), 
    (SELECT id FROM "COURSE" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io')), '2024-11-24 17:10:25', '即將授課'
);
